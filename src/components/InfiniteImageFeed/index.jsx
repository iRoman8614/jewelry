...
const defaultMockImages = Array.from({ length: 10 }, (_, i) => `https://source.unsplash.com/random/200x150?sig=${i + 20}`);

const ImageFeedWithPopup = ({
            ...
            <div className={styles.feedContainer} style={containerStyle}>
                <div className={styles.imagesWrapper} style={wrapperStyle}>
                    {duplicatedImages.map((src, index) => (
                        <div
                            key={`feed-img-${index}-${src.slice(-10)}`}
                            className={`${styles.imageItemContainer} ${
                                (index % images.length) % 2 === 0 ? styles.even : styles.odd
                            }`}
                            onClick={() => handleImageClick(src)} // Добавляем обработчик клика
                        >
                            <img
                                src={src}
            ...
            {isPopupOpen && selectedImage && (
                <div className={styles.popupOverlay} onClick={handleClosePopup}>
                    <div className={styles.popupContent} onClick={(e) => e.stopPropagation()}>
                        ...
                        <img src={selectedImage} alt="Selected feed image" className={styles.popupImage} />
                    </div>
                </div>
            )}
        </>
...
export default ImageFeedWithPopup;